...
class DatabaseLoggingHandler(logging.Handler):
    ...
    def emit(self, record):
        try:
            ...
            if not retrieve_database_exist(system=self.system,dbName=globalvar.PAAS_DASHBOARD_DBNAME["POSTGRES"][self.system],forRawData="postgres")[0]:
                create_database(self.system,globalvar.PAAS_DASHBOARD_DBNAME["POSTGRES"][self.system],forRawData="postgres")

            DbSessionRaw,metaRaw,engineRaw = appPaaS.getDbSessionType(dbName=globalvar.PAAS_DASHBOARD_DBNAME["POSTGRES"][self.system], system=self.system, forRawData="postgres")
            if not DbSessionRaw is None:
                ...
                if not retrieve_table_exist(metaRaw, tableName, self.system, doLoggerHandler=False):
                    resultRaw ,mesgRaw = create_table(table_name=tableName, dbName=globalvar.PAAS_DASHBOARD_DBNAME["POSTGRES"][self.system], attrList=table_attr, table_comment=self.system.lower()+"_PaaS API日誌",doLoggerHandler=False,system=self.system)
                    # print "~~~resultRaw~~~~"
                    # print resultRaw
                    # print "~~~mesgRaw~~~~"
                    # print mesgRaw
