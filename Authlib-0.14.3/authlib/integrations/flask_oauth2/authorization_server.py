...
class AuthorizationServer(_AuthorizationServer):
    ...
    def handle_response(self, status_code, payload, headers):
        ...
        if isinstance(payload, dict):
            #Yishan add for assort -> Response:ok
            if status_code == 200:
...
def create_token_expires_in_generator(expires_in_conf=None):
    ...
    def expires_in(client, grant_type):
        #Yishan add update again
        data.update(BearerToken.GRANT_TYPES_EXPIRES_IN)
        #print data
        #print "~~~~~~~~data.get(grant_type, BearerToken.DEFAULT_EXPIRES_IN)~~~~~~"
        #print data.get(grant_type, BearerToken.DEFAULT_EXPIRES_IN)
